...
interface Props {
  className?: string;
  article: NonNullable<Awaited<ReturnType<typeof getBlogArticles>>[number]>;
}
    ...
    >
      <article className="relative flex h-full w-full flex-col items-start justify-end overflow-hidden">
        <figure className="absolute bottom-0 left-0 right-0 top-0 -z-20">
          <Image fill objectFit="cover" alt={article.cover.alternativeText || article.title!} src={article.cover.url} />
        </figure>
