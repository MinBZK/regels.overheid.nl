...
export const strapiApiTokenPermissionsTokenLinks = pgTable("strapi_api_token_permissions_token_links", {
	id: serial("id").primaryKey().notNull(),
	apiTokenPermissionId: integer("api_token_permission_id").references(() => strapiApiTokenPermissions.id, { onDelete: "cascade" } ),
...
export const strapiTransferTokenPermissionsTokenLinks = pgTable("strapi_transfer_token_permissions_token_links", {
	id: serial("id").primaryKey().notNull(),
	transferTokenPermissionId: integer("transfer_token_permission_id").references(() => strapiTransferTokenPermissions.id, { onDelete: "cascade" } ),
...
(table) => {
	return {
		...
		orderInvFk: index("strapi_transfer_token_permissions_token_links_order_inv_fk").on(table.transferTokenPermissionOrder),
...
(table) => {
	return {
		...
		methodsLocalizationsLinksUnique: unique("methods_localizations_links_unique").on(table.methodId, table.invMethodId),
	}
});
